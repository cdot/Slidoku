...
const COLS = DIGITS;           // Column labels

const MIN_GIVENS = 17;         // Minimum number of givens (found empirically?)
const NR_SQUARES = DIM * DIM;  // Number of squares
...
class Sudoku {
  ...
  constructor(){
    ...
    this.SQUARE_UNITS_MAP    = this._get_square_units_map(
      this.SQUARES, this.UNITS);
    this.SQUARE_PEERS_MAP    = this._get_square_peers_map(
      this.SQUARES, this.SQUARE_UNITS_MAP);
  }

  // Generate
  // -------------------------------------------------------------------------
  generate(difficulty, unique){
    /* Generate a new puzzle of a particular `difficulty`, e.g.,
  ...
  solve(board, reverse){
    ...
    */
    ...
    var nr_givens = 0;
    for(var i in board){
      if(board[i] !== this.BLANK_CHAR && this._in(board[i], DIGITS)){
        ++nr_givens;
      }
    }
   ...
   * {square: DIGITS} using recursive constraint
   * propagation. Return `false` if a contradiction is encountered
   */
  ...
  /**
   * Test if the given `board` is valid or not. If it's valid, return
  ...
  validate_board(board){
    ...
    for(var i in board){
      if (!this._in(board[i], DIGITS) && board[i] !== this.BLANK_CHAR){
        return `Invalid board character '${board[i]}' encountered at index ${i}`;
      }
    }
